...
const PROFILES_FILE = join(CONFIG_DIR, "pagespeed-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "pagespeed-cli");
...
export function getActiveProfile(): Profile | null {
  if (process.env.PAGESPEED_URL && process.env.PAGESPEED_TOKEN) {
    return { url: process.env.PAGESPEED_URL!, token: process.env.PAGESPEED_TOKEN! };
  }
