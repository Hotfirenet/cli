...
export function output(
  ...
  switch (format) {
    ...
    case "csv": printCsv(data, opts.fields, opts.noHeader ?? globalFlags.noHeader); break;
    case "yaml": printYaml(data, 0); break;
    default: printText(data, opts.fields, opts.noHeader ?? globalFlags.noHeader);
  }
}

function printJson(data: unknown): void {
  const envelope: JsonEnvelope = { ok: true, data };
  if (Array.isArray(data)) envelope.meta = { total: data.length };
  console.log(JSON.stringify(envelope, null, 2));
}

function printText(data: unknown, fields?: string[], noHeader?: boolean): void {
  if (Array.isArray(data)) {
    if (data.length === 0) { console.log(pc.dim("(no results)")); return; }
...
function formatCell(v: unknown): string {
  ...
  if (typeof v === "object") return pc.dim(JSON.stringify(v));
  const s = String(v);
  if (/^\d{4}-\d{2}-\d{2}T/.test(s)) return pc.dim(s.replace("T", " ").replace(/\.\d+Z$/, "Z"));
  return s;
}
...
function printTable(rows: Record<string, unknown>[], fields?: string[], noHeader?: boolean): void {
  ...
  const widths = cols.map((col) => {
    ...
    return Math.min(Math.max(col.length, ...values), 50);
  });

  if (!noHeader) {
    console.log(cols.map((c, i) => pc.bold(pc.white(c.padEnd(widths[i] ?? 10)))).join("  "));
  ...
  for (const row of rows) {
    const line = cols.map((c, i) => {
      ...
      const w = widths[i] ?? 10;
      if (raw.length > w) return formatted.slice(0, formatted.length - (raw.length - w) - 1) + pc.dim("…");
...
function printCsv(data: unknown, fields?: string[], noHeader?: boolean): void {
  if (!Array.isArray(data)) { console.log(JSON.stringify(data)); return; }
...
function printYaml(data: unknown, indent: number): void {
  ...
  if (Array.isArray(data)) {
    for (const item of data) {
      if (typeof item === "object" && item !== null) { console.log(`${pad}-`); printYaml(item, indent + 1); }
      else console.log(`${pad}- ${String(item)}`);
    }
  } else if (typeof data === "object" && data !== null) {
    for (const [k, v] of Object.entries(data)) {
      if (typeof v === "object" && v !== null) { console.log(`${pad}${k}:`); printYaml(v, indent + 1); }
      else console.log(`${pad}${k}: ${String(v)}`);
    }
  }
}
