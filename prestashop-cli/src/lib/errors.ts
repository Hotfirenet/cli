...
const SUGGESTIONS: Record<number, string> = {
  401: "Check your API key: prestashop-cli auth test",
  403: "Insufficient permissions. Check API key access rights in PS Back Office > Advanced Parameters > Webservice.",
  404: "Resource not found. Verify the ID exists.",
  500: "Server error. Verify the webservice is enabled in PS Back Office.",
  503: "Service unavailable. The webservice may be disabled.",
};
...
export function handleError(err: unknown, json = false): never {
  if (err instanceof CliError) {
    if (json) {
      ...
      if (err.suggestion) console.error(`${pc.dim("Suggestion:")} ${err.suggestion}`);
    }
  ...
  if (err instanceof Error) {
    ...
    if (json) {
      const envelope: ErrorEnvelope = {
        ok: false,
        error: { code: status ?? 1, message: err.message, ...(suggestion && { suggestion }) },
      };
      console.error(JSON.stringify(envelope, null, 2));
    } else {
      console.error(`${pc.red("Error")}: ${err.message}`);
      if (suggestion) console.error(`${pc.dim("Suggestion:")} ${suggestion}`);
    }
