...
const PROFILES_FILE = join(CONFIG_DIR, "prestashop-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  if (!existsSync(PROFILES_FILE)) return { default: null, profiles: {} };
...
/** Normalize URL: strip trailing slash, ensure path ends with /api */
...
export function listProfiles(): Record<string, Profile> { return loadProfiles().profiles; }
export function getDefaultProfileName(): string | null { return loadProfiles().default; }
export function getProfile(name: string): Profile | null { return loadProfiles().profiles[name] ?? null; }

export function getActiveProfile(): Profile | null {
  if (process.env.PRESTASHOP_URL && process.env.PRESTASHOP_TOKEN) {
    return { url: process.env.PRESTASHOP_URL, token: process.env.PRESTASHOP_TOKEN };
  }
...
// ─── Single-profile helpers (used by auth command) ───────────────────────────

export function getToken(): string | null { return getActiveProfile()?.token ?? null; }
export function getBaseUrl(): string { return getActiveProfile()?.url ?? "http://localhost/api"; }
