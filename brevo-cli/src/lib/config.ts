...
const PROFILES_FILE = join(CONFIG_DIR, "brevo-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "brevo-cli");
...
export function getActiveProfile(): Profile | null {
  if (process.env.BREVO_URL && process.env.BREVO_TOKEN) {
    return { url: process.env.BREVO_URL!, token: process.env.BREVO_TOKEN! };
  }
