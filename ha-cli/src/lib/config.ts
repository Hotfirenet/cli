...
const PROFILES_FILE = join(CONFIG_DIR, "ha-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "ha-cli");
...
// ─── ha-cli instance compatibility shims ─────────────────────────────────────

export function listInstances(): { name: string; url: string; token: string }[] {
  return Object.entries(listProfiles()).map(([name, p]) => ({ name, url: p.url, token: p.token }));
}
...
export function getActiveInstanceName(): string | null {
