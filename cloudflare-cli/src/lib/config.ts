...
const PROFILES_FILE = join(CONFIG_DIR, "cloudflare-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "cloudflare-cli");
...
export function getActiveProfile(): Profile | null {
  if (process.env.CLOUDFLARE_URL && process.env.CLOUDFLARE_TOKEN) {
    return { url: process.env.CLOUDFLARE_URL!, token: process.env.CLOUDFLARE_TOKEN! };
  }
