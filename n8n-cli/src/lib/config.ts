...
const PROFILES_FILE = join(CONFIG_DIR, "n8n-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "n8n-cli");
...
export function getActiveProfile(): Profile | null {
  if (process.env.N8N_URL && process.env.N8N_TOKEN) {
    return { url: process.env.N8N_URL!, token: process.env.N8N_TOKEN! };
  }
