...
const PROFILES_FILE = join(CONFIG_DIR, "supabase-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "supabase-cli");
...
export function getActiveProfile(): Profile | null {
  if (process.env.SUPABASE_URL && process.env.SUPABASE_TOKEN) {
    return { url: process.env.SUPABASE_URL!, token: process.env.SUPABASE_TOKEN! };
  }
...
export function getManagementToken(): string | null {
  ...
  return p?.management_token ?? null;
}
...
export function saveManagementToken(token: string): void {
  ...
  if (store.profiles[name]) {
    store.profiles[name].management_token = token;
    _saveProfiles(store);
  }
}

export function getProjectRef(): string | null {
  const url = getActiveProfile()?.url ?? "";
