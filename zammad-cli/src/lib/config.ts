...
const PROFILES_FILE = join(CONFIG_DIR, "zammad-cli-profiles.json");
...
function loadProfiles(): ProfilesStore {
  ...
  if (!existsSync(PROFILES_FILE)) {
    const legacyToken = join(CONFIG_DIR, "zammad-cli");
...
export function getActiveProfile(): Profile | null {
  if (process.env.ZAMMAD_URL && process.env.ZAMMAD_TOKEN) {
    return { url: process.env.ZAMMAD_URL!, token: process.env.ZAMMAD_TOKEN! };
  }
