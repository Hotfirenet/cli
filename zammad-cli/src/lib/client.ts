...
function buildHeaders(): Record<string, string> {
  ...
  if (!token) throw new CliError(401, "No API token. Run: zammad-cli auth set <token>");
  return {
    "Content-Type": "application/json",
    Accept: "application/json",
    "Authorization": token.startsWith("Token ") || token.startsWith("Bearer ") ? token : `Token token=${token}`,
  };
}
